...
def main(f_train,f_valid,f_pred):
    ...
    trainer.train()
    trainer.save_model('./TEMP_mymodell')
    ...
    #print(classification_report(valid_df['label'], valid_df['pred'], digits=3, target_names=mylabels))
    ...
    for row in valid_df.itertuples():
        ...
        with torch.no_grad():
            ...
            #print(f'label:{out_label} / pred:{out_pred} / sentence1:{sentence1} / sentence2:{sentence2}')
    ...
    df_gtruth = valid_df.filter(items=['label'])
    ...
    df_out.to_csv('zzz_finalresults.csv',index=False)
...
def _eval_model(mymodel,f_test):

    #test_df = pd.read_csv("./text/news_test.csv",encoding='UTF-8')
    test_df = pd.read_csv(f_test,encoding='UTF-8')    
...
def do_test(mymodel,f_test):
    ...
    #model = (AutoModelForSequenceClassification.from_pretrained(mymodel,num_labels=class_label.num_classes).to(device))
    model = (AutoModelForSequenceClassification.from_pretrained(mymodel).to(device))
    ...
    for row in df_test.itertuples():
        ...
        encoded_input = tokenizer(sentence1, sentence2, max_length=128,return_tensors='pt')
        model.eval()
    ...
    f_fin = file_base +'_predictiontest' + ext
...
if __name__ == "__main__":
    ...
    if args.dotest:
        ...
        main(f_train,f_validation,f_test)        
